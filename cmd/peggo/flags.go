...
func cosmosKeyringFlagSet() *pflag.FlagSet {
	...
	fs.String(
		flagCosmosFrom,
		"",
		"Specify the Cosmos validator key name or address. If specified, must exist in keyring, ledger or match the privkey",
	)
...
func ethereumKeyOptsFlagSet() *pflag.FlagSet {
	...
	fs.String(
		flagEthFrom,
		"",
		"Specify the Ethereum from address; If specified, it must exist in the keystore, ledger or match the privkey",
	)
	fs.String(
		flagEthPassphrase,
		"",
		"Specify the passphrase to unlock the private key from armor; If empty then STDIN is used",
	)
...
func bridgeFlagSet() *pflag.FlagSet {
	...
	fs.Int64(
		flagEthGasPrice,
		0,
		"The Ethereum gas price to include in the transaction; If zero, gas price will be estimated",
	)
